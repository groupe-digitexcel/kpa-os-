      ...
      <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-sm border-t-4 border-kpa-gold">
        ...
        {mode === "password" ? (
          <form onSubmit={handleLogin}>
            ...
            <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 mb-4 focus:outline-none focus:ring-2 focus:ring-kpa-gold" />
            <label className="block text-sm font-medium text-kpa-navy mb-1"><BilingualText fr="Mot de passe" en="Password" /></label>
            <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 mb-6 focus:outline-none focus:ring-2 focus:ring-kpa-gold" />
            <button type="submit" disabled={loading} className="w-full bg-kpa-navy text-white font-semibold py-2.5 rounded-lg hover:bg-kpa-navy/90 disabled:opacity-50">
          ...
          <form onSubmit={handlePinLogin}>
            ...
            <select value={selectedStaffId} onChange={(e) => setSelectedStaffId(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 mb-4 focus:outline-none focus:ring-2 focus:ring-kpa-gold">
              {pinStaff.map((s: any) => {
            ...
            <input type="password" inputMode="numeric" required value={pin} onChange={(e) => setPin(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 mb-6 tracking-widest focus:outline-none focus:ring-2 focus:ring-kpa-gold" />
            <button type="submit" disabled={loading || pinStaff.length === 0} className="w-full bg-kpa-navy text-white font-semibold py-2.5 rounded-lg hover:bg-kpa-navy/90 disabled:opacity-50">
              {loading ? "..." : <BilingualText fr="Entrer avec le PIN" en="Enter with PIN" />}
            </button>
            {pinStaff.length === 0 && <p className="text-xs text-gray-400 mt-3 text-center"><BilingualText fr="Aucun membre du personnel n’a encore configuré de PIN sur cet ordinateur. Connectez-vous une fois avec votre e-mail et votre mot de passe, puis configurez-le depuis le tableau de bord." en="No one has set up a PIN on this computer yet. Log in once with email/password, then set one up from the dashboard." /></p>}
          </form>
        )}

        <button onClick={() => setMode(mode === "password" ? "pin" : "password")} className="w-full text-xs text-kpa-navy underline mt-4">
          {mode === "password" ? <BilingualText fr="Utiliser plutôt le PIN hors ligne" en="Use offline PIN instead" /> : <BilingualText fr="Utiliser plutôt l’e-mail et le mot de passe" en="Use email/password instead" />}
        </button>
      </div>
    </main>
  );
