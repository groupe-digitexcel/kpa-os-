...
export default function LoginPage() {
  ...
  async function handleBootstrap(e: React.FormEvent) {
    ...
    setError(null);
    if (pin !== confirmPin) {
      setError(language === "fr" ? "Les deux PIN ne correspondent pas." : "The PINs do not match.");
      setLoading(false);
      return;
    }
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-kpa-cream p-6 relative">
      <div className="absolute top-4 right-4 bg-kpa-navy rounded-lg">
        <LanguageSwitcher />
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-sm border-t-4 border-kpa-gold">
        ...
        <p className="text-sm text-gray-500 mb-4">
          {local ? (
            <BilingualText fr="Application de bureau hors ligne" en="Offline Desktop Application" />
          ) : (
            <BilingualText fr="Portail du Personnel" en="Staff Portal" />
          )}
        </p>
        ...
        {canBootstrap ? (
          <div>
            {testMode && (
              <button
                ...
                disabled={loading}
                className="w-full bg-kpa-gold text-kpa-navy font-bold py-3 rounded-lg mb-4 disabled:opacity-50"
              >
                {loading ? "..." : (
                  <BilingualText
                    fr="🧪 Ouvrir en mode TEST — Super Administrateur"
                    en="🧪 Open TEST MODE — Super Administrator"
                  />
                )}
              </button>
            )}
            ...
            <form onSubmit={handleBootstrap}>
              <div className="bg-green-50 text-green-800 text-sm p-3 rounded-lg mb-4">
                <BilingualText
                  fr="Ce PC n’est pas encore initialisé. Créez le premier Super Administrateur hors ligne. Aucun compte Internet n’est requis."
                  en="This PC is not initialized yet. Create the first Super Administrator offline. No online account is required."
                />
              </div>

              <label className="block text-sm font-medium text-kpa-navy mb-1">
                <BilingualText fr="Nom du Super Administrateur" en="Super Administrator name" />
              </label>
              <input
                required
                value={adminName}
              ...
              <label className="block text-sm font-medium text-kpa-navy mb-1">
                <BilingualText fr="Créer le PIN" en="Create PIN" />
              </label>
              <input
                required
                type="password"
                ...
                className="w-full border border-gray-300 rounded-lg px-3 py-2 mb-4 tracking-widest"
              />

              <label className="block text-sm font-medium text-kpa-navy mb-1">
                <BilingualText fr="Confirmer le PIN" en="Confirm PIN" />
              </label>
              <input
                required
                type="password"
                ...
                className="w-full border border-gray-300 rounded-lg px-3 py-2 mb-6 tracking-widest"
              />

              <button
                type="submit"
                disabled={loading}
                className="w-full bg-kpa-navy text-white font-semibold py-2.5 rounded-lg disabled:opacity-50"
              >
                {loading ? "..." : (
                  <BilingualText
                    fr="Initialiser KPA-OS hors ligne"
                    en="Initialize KPA-OS offline"
                  />
                )}
              </button>
            </form>
          ...
          <form onSubmit={handlePinLogin}>
            <label className="block text-sm font-medium text-kpa-navy mb-1">
              <BilingualText fr="Utilisateur" en="User" />
            </label>
            ...
            <label className="block text-sm font-medium text-kpa-navy mb-1">
              <BilingualText fr="PIN hors ligne" en="Offline PIN" />
            </label>
            ...
            {pinStaff.length === 0 && (
              <p className="text-xs text-gray-400 mt-3 text-center">
                <BilingualText
                  fr="Aucun PIN n’est configuré sur ce poste."
                  en="No PIN is configured on this computer."
                />
              </p>
            )}
          </form>
          ...
          <form onSubmit={handleLogin}>
            <label className="block text-sm font-medium text-kpa-navy mb-1">
              <BilingualText fr="E-mail" en="Email" />
            </label>
            ...
            <label className="block text-sm font-medium text-kpa-navy mb-1">
              <BilingualText fr="Mot de passe" en="Password" />
            </label>
            ...
            >
              {loading ? (
                <BilingualText fr="Connexion..." en="Signing in..." />
              ) : (
                <BilingualText fr="Se connecter" en="Login" />
              )}
            </button>
          </form>
        )}
        ...
        {!canBootstrap && local && (
          ...
          >
            <BilingualText
              fr={mode === "pin" ? "Utiliser e-mail/mot de passe (synchronisation)" : "Utiliser le PIN hors ligne"}
              en={mode === "pin" ? "Use email/password (sync)" : "Use offline PIN"}
            />
          </button>
        )}
      </div>
