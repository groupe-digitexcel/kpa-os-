...
export default async function GradebookPage() {
  ...
  if (!myClass) {
    return (
      <DashboardShell>
        <h1 className="text-2xl font-bold text-kpa-navy mb-1">Gradebook</h1>
  ...
  const [subjects, assessments] = await Promise.all([
    listSubjects(),
    listAssessments((myClass as any).id),
  ]);
  ...
  return (
    <DashboardShell>
      ...
      {assessments.length === 0 ? (
        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 text-sm text-gray-500 mb-6">
          No assessments yet — create your first one below.
        </div>
      ) : null}
