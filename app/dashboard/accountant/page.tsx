...
export default async function AccountantDashboard() {
  ...
  return (
    <DashboardShell>
      <h1 className="text-2xl font-bold text-kpa-navy mb-1">Accountant Overview</h1>
      <p className="text-sm text-gray-500 mb-4">Daily reconciliations awaiting approval</p>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 divide-y">
        {(!pending || pending.length === 0) && (
          <p className="p-5 text-sm text-gray-400">No pending reconciliations. All caught up.</p>
        )}
        {pending?.map((r: any) => (
          <div key={r.id} className="p-4 flex items-center justify-between text-sm">
            <div>
              ...
              <p className="text-gray-500">Submitted by {r.staff?.full_name}</p>
            </div>
            <div className="text-right">
              ...
              >
                Variance: {(r.variance_cash + r.variance_momo).toLocaleString()} XAF
              </p>
