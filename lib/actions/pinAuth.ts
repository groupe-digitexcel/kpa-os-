...
function hashPin(pin: string, salt: string) { return crypto.scryptSync(pin, salt, 32).toString("hex"); }
...
export async function bootstrapLocalAdmin(fullName: string, pin: string) {
  ...
  db.prepare(`INSERT INTO staff (id, auth_user_id, full_name, role, email, active, created_at, updated_at, deleted, pin_hash) VALUES (?, NULL, ?, 'super_admin', NULL, 1, ?, ?, 0, ?)`).run(id, name, now, now, makePinHash(pin));
  db.prepare(`UPDATE sync_meta SET value = ? WHERE key = 'current_staff_id'`).run(id);
  db.prepare(`INSERT INTO audit_log (id, actor_id, action, entity, entity_id, details, created_at) VALUES (?, ?, 'local_bootstrap', 'staff', ?, ?, ?)`).run(newId(), id, id, JSON.stringify({ role: "super_admin", offline: true }), now);
  const cookieStore = await cookies();
  cookieStore.set("local_pin_session", await signLocalSession(id, "super_admin"), { httpOnly: true, secure: false, sameSite: "lax", maxAge: 60 * 60 * 12, path: "/" });
...
export async function pinLogin(staffId: string, pin: string) {
  ...
  db.prepare(`UPDATE sync_meta SET value = ? WHERE key = 'current_staff_id'`).run(staff.id);
  const cookieStore = await cookies();
  cookieStore.set("local_pin_session", await signLocalSession(staff.id, staff.role), { httpOnly: true, secure: process.env.DATA_MODE !== "local", sameSite: "lax", maxAge: 60 * 60 * 12, path: "/" });
  return { success: true, staffId: staff.id };
}
