      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 divide-y">
        {roster.map((s) => (
          <button
            key={s.id}
            onClick={() => toggle(s.id)}
            className="w-full flex items-center justify-between px-4 py-3 text-sm hover:bg-kpa-cream text-left"
          >
            <span className="text-kpa-navy font-medium">{s.full_name}</span>
            <span
              className={`text-xs font-semibold px-3 py-1 rounded-full ${
                s.present ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"
              }`}
            >
        ...
        {roster.length === 0 && (
          <p className="p-5 text-sm text-gray-400">No students in this class.</p>
        )}
      </div>
    </div>
  );
