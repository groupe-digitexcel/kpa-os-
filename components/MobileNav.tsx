...
type NavItem = { fr: string; en: string; href: string };
    ...
    <>
      <div className="md:hidden sticky top-0 z-40 bg-kpa-navy text-white flex items-center justify-between px-4 py-3 shadow-md">
        ...
        <div className="flex items-center gap-2">
          <LanguageSwitcher />
          <button onClick={() => setOpen(true)} aria-label="Menu" className="w-9 h-9 flex items-center justify-center rounded-lg bg-white/10">
        ...
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          <div className="w-72 max-w-[80vw] bg-kpa-navy text-white p-5 overflow-y-auto flex flex-col">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setOpen(false)} aria-label="Fermer" className="w-8 h-8 flex items-center justify-center rounded-lg bg-white/10 text-lg">✕</button>
            </div>
            <nav className="flex flex-col gap-1 text-sm">
              {nav.map((item) => (
                <a key={item.href} href={item.href} onClick={() => setOpen(false)} className="px-3 py-2.5 rounded-lg hover:bg-white/10 active:bg-white/15 transition">
