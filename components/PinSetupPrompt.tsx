  ...
  return (
    <div className="bg-white rounded-xl p-4 shadow-sm border border-kpa-gold/40 mb-6">
      <div className="flex items-start justify-between mb-2">
        <p className="text-sm font-semibold text-kpa-navy">Set up offline PIN login</p>
        <button onClick={() => setDismissed(true)} className="text-xs text-gray-400">
          Later
        </button>
      </div>
      <p className="text-xs text-gray-500 mb-3">
        Lets you start a fresh session on this computer even if the internet is down —
      ...
      <form onSubmit={handleSubmit} className="flex gap-2 items-end flex-wrap">
        <div>
          <label className="block text-xs text-gray-500 mb-1">PIN (4-6 digits)</label>
          <input
            type="password"
            inputMode="numeric"
            value={pin}
